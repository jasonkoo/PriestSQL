...
//		});
		...
		Util.ajax({
			...
			success: function(data){
				...
				var _data = [{id: -1, connectName: 'Hive'}];
				$.extend(_data, data.data);
				
				var _connSelName = store.get(UserName + 'connSelName') || 'Hive';
				var _connSelId = store.get(UserName + 'connSelId') || -1;
...
//		if(connId != -1){
//			_url = 'ConnectController/getDatabases';
//			_data = {
//				id: connId
//			}
//		}
...
//				$('#QueryHistoryList').html(_html);
				...
				$.each(data.data, function(k, v){
					...
					if(v.queryType == 'H'){
						...
						_type = 'Spark 2.0';
					}else if(v.queryType == 'SS'){
						_type = 'Spark 2.0 > Spark 1.6';
					}else if(v.queryType == 'SH'){
						_type = 'Spark 2.0 > MR';
					}else if(v.queryType == 'SSH'){
						_type = 'Spark 1.6 > MR';
					}
					...
					_html += '<tr>'
						...
						+'<td>'+ _type +'</td>'
						+'<td>'+_state+'</td>'
						+'<td><a href="javascript:;" class="detail" data-dbname="'+v.dbName+'" data-state="'+v.runningState+'" data-id="'+v.id+'" data-queryType="'+v.queryType+'" data-logKey="'+v.logKey+'" data-queryCountKey="'+v.queryCountKey+'" data-queryKey="'+v.queryKey+'" data-proxyUser="'+v.proxyUser+'" data-sql="'+encodeURIComponent(v.sql)+'">'+$.i18n.prop('showResult')+'</a></td>'
	                +'</tr>'
				})
...
//		}
	...
	function getDownloadKey(){
		...
		var _param = {
			queryType: QueryType[getActiveEditor()],
			sql: _sql,
			pageSize: 100,
			dbName: $('#DatabaseChosen .fixedVal').val(),
			queryKey: QueryKey[getActiveEditor()],
...
//			Avatar.push(['track', ['spark', $('#UserName').html(), 'spark_label', {arg1:0, arg2:0, arg3:0, arg4:0, arg5:0}, 0]]);
			
			QueryType[getActiveEditor()] = 'spark';
			var _sql = window[getActiveEditor()].getSelection();
			if(_sql == ''){
				_sql = window[getActiveEditor()].getValue();
			}
...
//			Avatar.push(['track', ['hive', $('#UserName').html(), 'hive_label', {arg1:0, arg2:0, arg3:0, arg4:0, arg5:0}, 0]]);
			
			QueryType[getActiveEditor()] = 'hive';
		...
		$('#HistoryList').on('click', '.detail', function(){
			...
			$('#UserSel').html(AppList[_appUser]).data('user', _appUser);
