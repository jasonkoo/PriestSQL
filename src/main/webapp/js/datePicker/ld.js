...
Dates.getPath = (function(){
    return config.path ? config.path : LD.getPath;
}());
...
Dates.run = function(options){
    ...
    as.elemv = /textarea|input/.test(elem.tagName.toLocaleLowerCase()) ? 'value' : options.elemv||'innerHTML';
...
Dates.viewDate = function(Y, M, D){
    ...
    as.month.value = Dates.ymd[0] + '.' + Dates.digit(Dates.ymd[1] + 1);
    ...
    //Added by
    var curdate = Dates.elem[as.elemv];
    var hms = Dates.hmsin;
    var ymd = [log.ymd[0],log.ymd[1]+1,log.ymd[2]];
    //var getDates = Dates.creation([log.ymd[0],log.ymd[1]+1,log.ymd[2]]);
    var getDates = Dates.parse(ymd, [hms[0].value, hms[1].value, hms[2].value]);
    if( curdate == getDates ){
        Dates.addClass(log.thisDay, "sel");
    }
};
...
Dates.viewYears = function(YY){
    ...
    S('#laydate_ys'+idx+'').innerHTML = str;
    Dates.each(S('#laydate_ys'+idx+' li'), function(i, elem){
...
Dates.orien = function(obj, pos){
	...
	typeof Dates.options.atfollow === 'function' && Dates.options.atfollow(obj, rect); 
};
...
/*    if ($$l.language == "ENGLISH") {
    ...
    table.id = (table.className = 'laydate_table') + idx;
    tr = view = null;
	...
	*/
    ...
    if(!Dates.box){
        ...
        div.innerHTML =  log.html = '<div class="laydate_top">'
          +'<div class="laydate_ym laydate_y" id="laydate_YY'+idx+'">'
            +'<a class="laydate_choose laydate_chprev laydate_tab"><cite></cite></a>'
            +'<input id="laydate_y'+idx+'" readonly><label></label>'
            ...
            +'<div class="laydate_yms">'
              ...
              +'<ul id="laydate_ys'+idx+'"></ul>'
              +'<a class="laydate_tab laydate_chdown"><cite></cite></a>'
            +'</div>'
          +'</div>'
          +'<div class="laydate_ym laydate_m" id="laydate_MM'+idx+'">'
            +'<a class="laydate_choose laydate_chprev laydate_tab"><cite></cite></a>'
            +'<input id="laydate_m'+idx+'" readonly><label></label>'
            +'<a class="laydate_choose laydate_chnext laydate_tab"><cite></cite></a>'
            +'<div class="laydate_yms" id="laydate_ms'+idx+'">'+ function(){
                var str = '';
                Dates.each(new Array(12), function(i){
                    str += '<span m="'+ i +'">'+ Dates.digit(i+1) +yue+'</span>';
                });
                return str;
            }() +'</div>'
          +'</div>'
        ...
        +'<div class="laydate_bottom">'
          +'<ul id="laydate_hms'+idx+'">'
            +'<li class="laydate_sj">时间</li>'
            +'<li><input readonly>:</li>'
            +'<li><input readonly>:</li>'
            +'<li><input readonly></li>'
          +'</ul>'
          +'<div class="laydate_time" id="laydate_time'+idx+'"></div>'
          +'<div class="laydate_btn">'
            +'<a id="laydate_clear'+idx+'">'+qk+'</a>'
            +'<a id="laydate_today'+idx+'">'+jt+'</a>'
            +'<a id="laydate_ok'+idx+'">'+qr+'</a>'
          +'</div>'
...
//关闭控件
Dates.close = function(reason){
	if(reason=='docclk'){
		var noclose = false; 
	}else{
		var noclose = typeof Dates.options.bfclose === 'function' && Dates.options.bfclose(reason); 
	}
	if( !noclose ){
		Dates.reshow();
...
Dates.events = function(){
    ...
    as.tds = S('#laydate_table'+idx+' td');
    as.mms = S('#laydate_ms'+idx+' span');
    as.year = S('#laydate_y'+idx+'');
    as.month = S('#laydate_m'+idx+'');
    ...
    Dates.each(S('#laydate_YY'+idx+' .laydate_tab'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            Dates.stopmp(ev);
            log.tabYear(i);
        });
    ...
    Dates.each(S('#laydate_MM'+idx+' .laydate_tab'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            Dates.stopmp(ev).reshow();
            log.tabMonth(i);
        });
    ...
    //选择月
    Dates.each(S('#laydate_ms'+idx+' span'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            Dates.stopmp(ev).reshow();
    ...
    //选择日
    Dates.each(S('#laydate_table'+idx+' td'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            if(!Dates.hasClass(this, as[1])){
                Dates.stopmp(ev);
    ...
    //确认
    as.ok = S('#laydate_ok'+idx+'');
    Dates.on(as.ok, 'click', function(){
        if(Dates.valid){
            //var getDates = Dates.creation([Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]]);
			typeof Dates.options.okclick === 'function' && Dates.options.okclick(); 
        }
    });
    
    //选择时分秒
    log.times = S('#laydate_time'+idx+'');
    Dates.hmsin = log.hmsin = S('#laydate_hms'+idx+' input');
    log.hmss = ['小时', '分钟', '秒数'];
    log.hmsarr = [];
    ...
    Dates.msg = function(i, title){
        ...
        if(typeof i === 'string'){
            ...
            if(!log.hmsarr[i]){
                str += '<div id="laydate_hmsno'+idx+'" class="laydate_hmsno">';
    ...
    log.hmson = function(input, index){
        var span = S('#laydate_hmsno'+idx+' span'), set = Dates.valid ? null : 1;
    ...
    Dates.on(doc, 'mouseup', function(){
        ...
        if(box && box.style.display !== 'none'){
            Dates.check() || Dates.close('docclk');
        }
...
Dates.init = (function(){
    var need = Dates.query('#LayDateSkinBase');
	if( !need ){
		Dates.use('need', 'LayDateSkinBase');
	}
    var sl = Dates.query('#'+as[3]);
    if( !sl ){
